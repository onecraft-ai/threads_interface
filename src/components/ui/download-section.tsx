...
export function DownloadSection() {
  ...
  // Найти релиз для текущей ОС пользователя
  const assetsWithPlatformInfo = release.assets.map(asset => ({
    ...asset,
    platformInfo: getPlatformInfo(asset.name)
  }));

  const primaryAsset = assetsWithPlatformInfo.find(asset => 
    asset.platformInfo.osType === userOS
  ) || assetsWithPlatformInfo[0];

  const otherAssets = assetsWithPlatformInfo.filter(asset => 
